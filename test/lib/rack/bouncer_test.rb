...
class Rack::Bouncer::Test < Test::Unit::TestCase

  def test_redirects_to_where_it_should_if_ie6
    request  = Rack::MockRequest.new(Rack::Bouncer.new(TestApp.new, {:redirect => 'http://slashdot.org'}))
  ...
  def test_redirects_to_where_it_should_if_user_specified_minimum_not_met
    request  = Rack::MockRequest.new(Rack::Bouncer.new(TestApp.new, {:redirect => 'http://slashdot.org', :minimum => 6.0}))
  ...
  def test_redirects_to_local_urls
    request  = Rack::MockRequest.new(Rack::Bouncer.new(TestApp.new, {:redirect => '/foo'}))
